...
namespace Hexio.DineroClient.Tests
{
    public class QueryCreatorTests
    {
        ...
        public void Test_QueryCreatorFormatIsCorrect()
        {
            ...
            queryCreator.ToString().Should().Be($"changesSince={dateString}");

            queryCreator.Where(x => x.Name, QueryOperator.Eq, "Morten");
            
            queryCreator.ToString().Should().Be($"queryFilter=Name+eq+'Morten'&changesSince={dateString}");
            
            queryCreator.Include(x => x.Name);
            
            queryCreator.ToString().Should().Be($"fields=Name&queryFilter=Name+eq+'Morten'&changesSince={dateString}");
        }
    }
}